...
static int mov_write_tai_timestamp_packet(AVIOContext *pb, MOVMuxContext *mov, int frame_number) {
    uint64_t pos;
    pos = avio_tell(pb);

    printf("writing timestamp %d at %lu\n", frame_number, pos);
...
static int mov_write_tai_timestamps(AVIOContext *pb, MOVMuxContext *mov, AVFormatContext *s) {
    // Write the timestamp values in the mdat. 
    // Save the pointer locations in mov->timestamp_offsets[]
...
static void mov_write_fullbox(AVIOContext * pb, uint8_t version, uint32_t flags) {
...
static int mov_write_saiz_box(AVIOContext *pb, MOVMuxContext *mov, AVFormatContext *s) {
    //Extends Full Box
    int64_t pos = avio_tell(pb);
    uint8_t version = 0;
    uint32_t flags = 1;
...
static int mov_write_saio_box(AVIOContext *pb, MOVMuxContext *mov, AVFormatContext *s) {
...
static int mov_write_itai_tag(AVIOContext *pb, MOVTrack *track)
{
    int64_t pos = avio_tell(pb);
    avio_wb32(pb, 0); /* size */
...
static int mov_write_taic_tag(AVIOContext *pb, MOVTrack *track)
{
    int64_t pos = avio_tell(pb);
    avio_wb32(pb, 0); /* size */
